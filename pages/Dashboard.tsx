...
import { Server, FileCode, CheckCircle, Activity } from 'lucide-react';
...
const Dashboard: React.FC<DashboardProps> = ({ workspaceId }) => {
  ...
  const cards = [
    { name: 'Agents', value: stats.agents, icon: Server, color: 'bg-blue-500', subtitle: `${stats.onlineAgents} Online` },
    { name: 'Templates', value: stats.templates, icon: FileCode, color: 'bg-indigo-500', subtitle: 'Docker Compose' },
    { name: 'Running Tasks', value: stats.runningTasks, icon: Activity, color: 'bg-amber-500', subtitle: 'Deployment status' },
    { name: 'Health Status', value: '100%', icon: CheckCircle, color: 'bg-emerald-500', subtitle: 'System operational' },
  ];
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {cards.map((card) => (
          <div key={card.name} className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 flex flex-col">
