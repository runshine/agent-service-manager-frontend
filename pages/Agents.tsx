...
const Agents: React.FC<AgentsProps> = ({ workspaceId, onSelectAgent }) => {
  ...
  const fetchAgents = async (isManualListRefresh = false) => {
    try {
      if (isManualListRefresh) setListRefreshing(true);
      else if (!refreshing) setLoading(true);

      const res = await api.agents.list(page, perPage, workspaceId);
      ...
      <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-4">
        <div>
          ...
          <p className="text-gray-500 text-sm font-medium">
            Registry-synchronized node management and infrastructure monitoring.
          </p>
        </div>
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="p-2 text-gray-500 hover:text-blue-600 hover:bg-blue-50 border border-gray-200 rounded-lg transition-all active:scale-95 disabled:opacity-50 shadow-sm"
          >
            <RefreshCw size={20} className={listRefreshing ? 'animate-spin' : ''} />
          </button>
          <button
            onClick={handleManualRefresh}
            disabled={refreshing}
            className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg flex items-center gap-2 transition-all disabled:opacity-50 shadow-lg shadow-blue-600/20 active:scale-[0.98] text-sm font-bold uppercase tracking-widest"
          >
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-gray-50/80 border-b border-gray-100">
                ...
                <th className="px-6 py-4 text-[10px] font-black text-gray-400 uppercase tracking-widest">Agent Version</th>
            ...
            <tbody className="divide-y divide-gray-50">
              {loading && !refreshing && !listRefreshing ? (
                <tr>
                  <td colSpan={7} className="px-6 py-24 text-center">
                    <div className="flex flex-col items-center gap-4">
                      ...
                      <p className="text-xs text-gray-400 font-black uppercase tracking-widest">Discovering network nodes...</p>
                    </div>
                  </td>
                </tr>
                ...
                const nacosVersion = agent.system_info?.formatted?.nacos_agent_version;
                const osVersion = agent.system_info?.os_version;
                const arch = agent.system_info?.architecture;
                    ...
                    <td className="px-6 py-4">
                      <div className="flex flex-col">
                        ...
                        <div className="flex items-center gap-1.5 mt-0.5">
                          <span className="text-[10px] text-gray-400 font-mono tracking-tighter uppercase font-medium">
                            ID: {agent.key.substring(0, 8)}...
                          </span>
                          {arch && (
                            <span className="text-[8px] bg-gray-100 text-gray-500 px-1 rounded uppercase font-bold">{arch}</span>
                          )}
                        </div>
                        {osVersion && (
                          <span className="text-[9px] text-gray-400 mt-1 font-medium">{osVersion}</span>
                        )}
                      </div>
                    </td>
                    <td className="px-6 py-4">
                      {nacosVersion ? (
                        <div className="flex flex-col gap-1">
                          <div className="flex items-center gap-1.5 text-[10px] font-bold text-blue-700 bg-blue-50 border border-blue-100 px-2 py-0.5 rounded-full w-fit shadow-sm">
                            <Tag size={10} className="text-blue-400" />
                            <span className="font-mono">{nacosVersion}</span>
                          </div>
                          <span className="text-[8px] text-gray-300 font-black uppercase tracking-widest pl-1">Build Timestamp</span>
                        </div>
                      ) : (
                        <span className="text-[10px] font-bold text-gray-300 uppercase italic tracking-widest">N/A</span>
                      )}
                    </td>
                    <td className="px-6 py-4">
                      <div className="flex flex-col">
                        <span className="text-xs font-mono font-bold text-gray-700 bg-gray-100 px-1.5 py-0.5 rounded w-fit">
                          {agent.ip_address}
                        </span>
                      ...
                      {agent.status === 'online' ? (
                        <div className="space-y-2">
                          ...
                          <ResourceBar 
                            ...
                            icon={Database} 
                            subtext={agent.system_info?.formatted?.memory ? `${agent.system_info.formatted.memory.used} / ${agent.system_info.formatted.memory.total}` : undefined}
                          />
                        </div>
                      ) : (
                        <span className="text-[10px] font-black text-gray-200 uppercase tracking-widest italic">Offline Data Only</span>
                      )}
                    </td>
                    <td className="px-6 py-4">
                        ...
                        {agent.services && agent.services.length > 0 ? (
                          <>
                            {agent.services.slice(0, 2).map((svc, i) => (
                              <span key={i} className="text-[10px] font-bold bg-white border border-gray-100 text-gray-600 px-2 py-0.5 rounded-md shadow-sm flex items-center gap-1 transition-all hover:border-blue-200">
                          ...
                          <span className="text-[10px] text-gray-300 font-bold uppercase tracking-widest">No workloads</span>
                        )}
                      </div>
                    </td>
                    <td className="px-6 py-4 text-right">
                      <div className="flex justify-end gap-2">
                        ...
                        <button
                          ...
                          className="p-2 text-gray-400 hover:text-emerald-600 hover:bg-emerald-50 rounded-lg transition-all disabled:opacity-20 active:scale-95"
                          title="Quick Deploy"
                        >
        ...
        <div className="px-6 py-4 bg-gray-50 border-t border-gray-200 flex flex-col sm:flex-row justify-between items-center gap-4">
          <div className="flex items-center gap-4">
            <div className="text-[10px] font-black text-gray-400 uppercase tracking-widest">
              Total Managed Nodes: <span className="text-gray-900">{total}</span>
            </div>
            <div className="flex items-center gap-2">
              ...
              <select 
                ...
                className="text-xs font-black bg-white border border-gray-200 rounded px-2 py-1 focus:outline-none focus:ring-2 focus:ring-blue-500/20 shadow-sm cursor-pointer"
              >
            ...
            <div className="p-8 space-y-6">
              <div>
                ...
                <input
                  ...
                  className="w-full px-5 py-3.5 bg-gray-50 border-2 border-gray-100 rounded-2xl focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 focus:outline-none transition-all font-bold text-gray-900"
              ...
              <div>
                ...
                <select
                  className="w-full appearance-none px-5 py-3.5 bg-gray-50 border-2 border-gray-100 rounded-2xl focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 focus:outline-none transition-all font-black text-gray-800 cursor-pointer"
              ...
              <div className="pt-6 flex gap-4">
                <button
                  ...
                  className="flex-1 px-6 py-4 border-2 border-gray-100 text-gray-500 rounded-2xl hover:bg-gray-50 hover:text-gray-700 transition-all font-black text-xs tracking-widest uppercase"
                >
                  CANCEL
                </button>
                <button
                  ...
                  className="flex-1 px-6 py-4 bg-blue-600 text-white rounded-2xl hover:bg-blue-700 disabled:opacity-50 disabled:grayscale transition-all font-black text-xs tracking-widest uppercase shadow-xl shadow-blue-500/20 active:scale-[0.98]"
                >
                  DEPLOY NOW
                </button>
              </div>
...
const ResourceBar = ({ label, value, icon: Icon, subtext }: { label: string, value: number, icon: any, subtext?: string }) => (
    ...
    <div className="h-1.5 w-full bg-gray-100 rounded-full overflow-hidden">
      <div 
        className={`h-full transition-all duration-700 ease-out ${
          value > 90 ? 'bg-red-500 shadow-[0_0_8px_rgba(239,68,68,0.5)]' : 
          value > 70 ? 'bg-amber-500' : 'bg-blue-500'
        }`}
        style={{ width: `${Math.min(100, Math.max(0, value))}%` }}
      />
    </div>
    {subtext && (
      <div className="text-[8px] text-gray-300 font-bold uppercase tracking-tighter text-right leading-none mt-0.5">
        {subtext}
      </div>
    )}
  </div>
